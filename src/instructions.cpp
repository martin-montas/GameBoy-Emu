...
void InstructionSet::pre_boot_state()
{
	...
	_bus->write8(0xFF02,
		     0x7E); // SC (Serial Link Control - Unused bits read as 1)
	...
	// 5. Unmap Boot ROM
	// _bus->write8(0xFF50, 0x01); // Unmaps Boot ROM, enabling Cartridge
	// mapping
...
void InstructionSet::execute(uint8_t opcode)
{
	...
	case 0x09: {
		// printf("ADD HL, BC -- %X --\n", _cpu->HL);
		_cpu->set_flag(FLAG_CARRY, (static_cast<uint32_t>(_cpu->HL) +
					    _cpu->BC) > 0xFFFF);
		_cpu->set_flag(FLAG_SUBTRACT, false);
		_cpu->set_flag(FLAG_HALF_CARRY, ((_cpu->HL & 0x0FFF) +
						 (_cpu->BC & 0x0FFF)) > 0x0FFF);
	...
	case 0xE8: {
		...
		_cpu->set_flag(FLAG_HALF_CARRY,
			       ((old_sp & 0xF) + (s8 & 0xF)) > 0xF);
		_cpu->set_flag(FLAG_CARRY,
			       ((old_sp & 0xFF) + (s8 & 0xFF)) > 0xFF);
		_cpu->SP = result;
	...
	case 0xF8: {
		...
		_cpu->clear_flag(FLAG_SUBTRACT);
		_cpu->set_flag(FLAG_HALF_CARRY,
			       (_cpu->SP & 0x0F) + (s8 & 0x0F) > 0x0F);
		_cpu->set_flag(FLAG_CARRY,
			       ((_cpu->SP & 0xFF) + (s8 & 0xFF)) > 0xFF);
		break;
	}
	case 0xF9: {
		// printf("  LDSP, HL\n");
...
void InstructionSet::adc(uint8_t& reg_1, uint8_t reg_2)
{
	...
	_cpu->clear_flag(FLAG_SUBTRACT);
	_cpu->set_flag(FLAG_HALF_CARRY,
		       ((reg_1 & 0x0F) + (reg_2 & 0x0F) + old_c) > 0x0F);
