...
void InstructionSet::execute(uint8_t opcode) {
  ...
  case 0x01: {
    std::cout << "LD BC, d16 0x01" << std::endl;
    ldr(cpu->BC);
    cpu += 3;
    break;
  }
  ...
  case 0x03: {
    std::cout << "INC BC 0x03" << std::endl;
    cpu->BC++;
    break;
  }
  case 0x04: {
    std::cout << "INC B 0x04" << std::endl;
    inc(cpu->B);
    cpu->PC = cpu->PC + 1;
    break;
  }
  case 0x05: {
    std::cout << "DEC B 0x05" << std::endl;
    dec(cpu->B);
    cpu->PC = cpu->PC + 1;
    break;
  }
  ...
  case 0x08: {
    ...
    uint16_t address =
        mmu->romData[cpu->PC + 1] | (mmu->romData[cpu->PC + 2] << 8);
  ...
  case 0x77: {
    // soon
    std::cout << "LD (HL), A 0x77" << std::endl;
...
void inc_mem(uint8_t value) { return; }
