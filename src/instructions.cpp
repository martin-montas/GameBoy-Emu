...
uint16_t InstructionSet::ldr(uint16_t reg) {
...
void InstructionSet::execute(uint8_t opcode) {
  ...
  case 0x03: {
    // DONE
    printf("INC BC 0x03 -- before %X --", cpu->BC);
    cpu->BC = cpu->BC + 1;
    cpu->PC = cpu->PC + 1;
    break;
  }
  case 0x04: {
    // DONE
    printf("INC B 0x04 -- before %X --", cpu->B);
    inc(&cpu->B);
    cpu->PC = cpu->PC + 1;
    break;
  }
  case 0x05: {
    // DONE
    printf("DEC B 0x05 -- before %X --", cpu->B);
    dec(&cpu->B, 1);
    cpu->PC = cpu->PC + 1;
    break;
  }
  case 0x06: {
    // DONE:
    printf("LD B, d8 0x06 --- before %X --", cpu->B);
