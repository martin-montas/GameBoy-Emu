...
void Ppu::bg_update_framebuff(uint16_t addr) {
    ...
    for (int t = 0; t <= 19; t++) {
        ...
        /* gets tile index out of tile map */
        uint8_t tile_index = _mmu->read8(addr + (32 * start_tile_y) + t);

        /* signed 8 bit integer for the indexing tile data */
        uint16_t tile_data;

        /* gets tile data based on the indexing method  in LCDC */
        // if ((LCDC & FLAG_BG_AREA) == 0) {
        tile_data = 0x8000 + (tile_index * 16);
        // } else {
        // tile_data = 0x9000 + (tile_index * 16);
        // }
        ...
        for (int k = 7; k >= 0; k--) {
            ...
            frame_buff[LY * WIDTH + (t * 8) + k] = color_val;
        }
    }
}
